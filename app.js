...
const pricingSimulator = {
  ...
  pricingMatrix: {
    coupe_sedan: {
      ...
      full_detail: { priceGs: 250000, durationMin: 150 },
      full_interior: { priceGs: 180000, durationMin: 110 },
      exterior_detail: { priceGs: 140000, durationMin: 80 },
      ceramic_coating: { priceGs: 450000, durationMin: 240 }
    },
    suv_mediana: {
      name: 'SUV Mediana (5 Asientos)',
      wizardCat: 'suv_mediana',
      full_detail: { priceGs: 300000, durationMin: 180 },
      full_interior: { priceGs: 200000, durationMin: 120 },
      exterior_detail: { priceGs: 160000, durationMin: 90 },
      ceramic_coating: { priceGs: 550000, durationMin: 270 }
    },
    pickup_suv_grande: {
      name: 'SUV Grande / Pick-Up (7 Asientos)',
      wizardCat: 'pickup_suv_grande',
      full_detail: { priceGs: 350000, durationMin: 210 },
      full_interior: { priceGs: 240000, durationMin: 145 },
      exterior_detail: { priceGs: 190000, durationMin: 105 },
      ceramic_coating: { priceGs: 650000, durationMin: 300 }
    },
    van_oversized: {
      name: 'Van / Furgón Grande',
      wizardCat: 'pickup_suv_grande',
      full_detail: { priceGs: 400000, durationMin: 240 },
      full_interior: { priceGs: 280000, durationMin: 170 },
      exterior_detail: { priceGs: 220000, durationMin: 120 },
      ceramic_coating: { priceGs: 750000, durationMin: 330 }
    }
  },

  // Package definitions & UI mappings
  packageMeta: {
    full_detail: {
      name: 'Full Detail (Exterior + Interior)',
      cardId: 'pkgCardFullDetail',
      priceId: 'pkgPriceFullDetail',
    ...
    exterior_detail: {
      ...
      cardId: 'pkgCardExterior',
      priceId: 'pkgPriceExterior',
      btnTextId: 'pkgBtnTextExterior'
    },
    ceramic_coating: {
      name: 'Ceramic Coating',
      cardId: 'pkgCardCeramic',
      priceId: 'pkgPriceCeramic',
      btnTextId: 'pkgBtnTextCeramic'
    }
  },
  ...
  selectVehicleSize(size) {
    ...
    // Update prices on all 4 package cards
    const pkgData = this.pricingMatrix[size];
    Object.keys(this.packageMeta).forEach(pkgKey => {
      const meta = this.packageMeta[pkgKey];
      const priceEl = document.getElementById(meta.priceId);
      if (priceEl && pkgData[pkgKey]) {
        priceEl.innerText = `Gs. ${pkgData[pkgKey].priceGs.toLocaleString("es-PY")}`;
      }
    });
  ...
  selectPackage(pkg) {
    if (!this.packageMeta[pkg]) return;
    this.state.package = pkg;

    // Update all package cards UI
    Object.keys(this.packageMeta).forEach(pkgKey => {
      const meta = this.packageMeta[pkgKey];
      const card = document.getElementById(meta.cardId);
      const btnText = document.getElementById(meta.btnTextId);

      if (card) {
        card.classList.toggle("selected", pkgKey === pkg);
      }
      if (btnText) {
        btnText.innerText = pkgKey === pkg ? "Paquete Seleccionado" : "Elegir este Paquete";
      }
    });
  ...
  renderSummaryStage() {
    ...
    const basePkg = vehConfig[this.state.package] || vehConfig.full_detail;
